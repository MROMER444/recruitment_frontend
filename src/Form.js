...
const Form = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        formData.append('Short_Introduction', introduction);
        formData.append('age', age);
        formData.append('city', city);
        formData.append('field_of_study', fieldOfStudy);
        formData.append('degree', degree);
        formData.append('year_of_experience', experience);
        formData.append('photo', photo);
        formData.append('resume', resume);

        try {
            const response = await fetch('http://localhost:5000/api/v1/information', {
                method: 'POST',
                body: formData
            });
            if (response.ok) {
